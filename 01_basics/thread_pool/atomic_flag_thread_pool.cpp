...
class HFTThreadPool {
public:
    explicit HFTThreadPool(size_t num_threads) {
...
int main() {
    HFTThreadPool pool(4);
    std::vector<std::future<int>> futures;
    std::cout << "HFT Threadpool 시작 (CPU 사용량 주의)\n";
    for (int i = 0; i < 100; i++)
    {
        futures.emplace_back(pool.EnqueueJob([i]() {
            int sum = 0;
