...
using namespace orderbook::v1;

// Generate random market data messages
...
public:
    ...
    // Generate with distribution: 70% Add, 20% Modify, 10% Delete
...
int main() {
    std::cout << "=== V1 Baseline Benchmark (std::mutex + std::map) ===" << std::endl;
    ...
    OrderBookHandler handler(latency);
    MessageGenerator generator;

    // Pre-generate all messages to avoid generation overhead during benchmark
    ...
    // Print results
    latency.printStats("V1 - std::mutex + std::map");
